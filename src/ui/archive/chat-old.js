...
class HLNAChat {
  ...
  generateHLNAResponse(userMessage) {
    ...
    if (this.personalityActive && this.currentPersonality) {
      responses = this.getPersonalityResponse(
        userMessage,
  ...
  getPersonalityResponse(message, personality) {
    ...
    const personalityResponses = {
      sherlock: {
        greeting:
          "Intéressant... 🔍 Permettez-moi d'analyser votre question avec ma méthode déductive habituelle.",
        style: 'analytique et méticuleux',
        emotion: 'analytical',
      },
      artist: {
        greeting:
          'Quelle inspiration ! 🎨 Laissez-moi peindre une réponse avec les couleurs de la créativité.',
        style: 'créatif et expressif',
        emotion: 'creative',
      },
      philosopher: {
        greeting:
          'Voilà une question qui mérite réflexion... 🤔 Explorons ensemble les profondeurs de cette pensée.',
        style: 'profond et contemplatif',
        emotion: 'curious',
      },
      scientist: {
        greeting:
          'Fascinant ! 🔬 Approchons ce sujet avec rigueur scientifique et méthode empirique.',
        style: 'rigoureux et méthodique',
        emotion: 'analytical',
      },
      poet: {
        greeting:
          'Vos mots résonnent en moi comme un vers... ✍️ Laissez-moi vous répondre en prose lyrique.',
        style: 'poétique et sensible',
        emotion: 'inspired',
      },
      explorer: {
        greeting:
          "Une nouvelle terre à découvrir ! 🗺️ Partons ensemble à l'aventure de cette idée.",
  ...
  getContextualResponse(message) {
    ...
    if (
      lowercaseMessage.includes('hlna') ||
      lowercaseMessage.includes('qui es-tu') ||
      lowercaseMessage.includes('qui êtes-vous')
    ) {
      return {
    ...
    if (
      lowercaseMessage.includes('inspire') ||
      lowercaseMessage.includes('extraordinaire')
    ) {
      return this.getInspirationalResponse(message);
    }

    if (
      lowercaseMessage.includes('histoire') ||
      lowercaseMessage.includes('raconte')
    ) {
      return this.getStoryResponse(message);
    }

    if (
      lowercaseMessage.includes('rêve') ||
      lowercaseMessage.includes('rêves')
    ) {
      return this.getDreamResponse(message);
    }

    if (
      lowercaseMessage.includes('personnalité') ||
      lowercaseMessage.includes('simule')
    ) {
  ...
  getPersonalityResponse(message) {
    ...
    const randomPersonality =
      personalities[keys[Math.floor(Math.random() * keys.length)]];
  ...
  getNormalResponse(lowercaseMessage) {
    ...
    if (
      ...
      lowercaseMessage.includes('analyser')
    ) {
      return {
  ...
  updateStats() {
    ...
    document.getElementById('learning-score').textContent = Math.round(
      this.learningScore
    );
    ...
    document.getElementById('message-progress').style.width =
      `${messageProgress}%`;
  ...
  switchMode(mode) {
    ...
    this.addSystemMessage(
      `🔄 Mode ${mode} activé ! Mon approche va s'adapter à ce style de conversation.`
    );
  ...
  setupAdvancedControls() {
    ...
    document
      .getElementById('personality-wheel')
      .addEventListener('click', () => {
        this.openPersonalityWheel();
      });

    document
      .getElementById('conversation-map')
      .addEventListener('click', () => {
  ...
  selectPersonality(personality) {
    ...
    const personalities = {
      sherlock: {
        ...
        emoji: '🔍',
        trait: 'analytique et déductif',
      },
      artist: {
        name: 'Un Artiste Visionnaire',
        emoji: '🎨',
        trait: 'créatif et inspirant',
      },
      philosopher: {
        name: 'Un Philosophe Sage',
        emoji: '🤔',
        trait: 'réfléchi et profond',
      },
      scientist: {
        name: 'Un Scientifique Curieux',
        emoji: '🔬',
        trait: 'rigoureux et méthodique',
      },
      poet: {
        name: 'Un Poète Sensible',
        emoji: '✍️',
        trait: 'expressif et émotionnel',
      },
      explorer: {
        name: 'Un Explorateur Aventurier',
        emoji: '🗺️',
        trait: 'audacieux et découvreur',
      },
    };

    const selected = personalities[personality];
    this.addSystemMessage(
      `🎭 HLNA adopte maintenant la personnalité de ${selected.name} ${selected.emoji} - ${selected.trait}`
    );
  ...
  generateConversationGraph() {
    ...
    nodes.forEach((node, index) => {
      ...
      circle.setAttribute(
        'fill',
        node.sender === 'user' ? '#4ECDC4' : '#FFD700'
      );
  ...
  triggerEmotion(emotion) {
    ...
    this.addSystemMessage(
      `💫 HLNA ressent maintenant : ${this.getEmotionDescription(emotion)} ${this.getEmotionEmoji(emotion)}`
    );
  ...
  openModal(content) {
    ...
    modalContent.innerHTML =
      content +
      '<button onclick="document.getElementById(\'modal-overlay\').style.display=\'none\'" style="margin-top: 20px; background: #FF6B6B; border: none; padding: 10px 20px; border-radius: 8px; color: white; cursor: pointer;">Fermer</button>';
  ...
  startBrainstormSession() {
    this.addSystemMessage(
      `🧠 Session de brainstorming activée ! HLNA est maintenant en mode hyper-créatif. Posez-moi n'importe quelle question ou défi, et explorons ensemble toutes les possibilités !`
    );
