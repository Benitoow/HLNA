...
class HLNAWebConnection {
  ...
  async fetchWeatherData(parameters) {
    ...
    const simulatedWeather = {
      ...
      condition: ['Ensoleillé', 'Nuageux', 'Pluvieux', 'Orageux'][
  ...
  async handleWebSearch(searchData) {
    ...
    document.dispatchEvent(
      new CustomEvent('hlna-search-results', {
        detail: results,
      })
    );
  }
  ...
  async handleRealTimeDataRequest(requestData) {
    ...
    document.dispatchEvent(
      new CustomEvent('hlna-realtime-data-response', {
        detail: data,
      })
    );
  }
  ...
  sendNotification(title, options = {}) {
    if (
      !this.webCapabilities.notifications ||
